...
public class DetailReservationScreen extends AppCompatActivity{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        imagePath.getDownloadUrl().addOnCompleteListener(new OnCompleteListener<Uri>() {
            ...
            public void onComplete(@NonNull Task<Uri> task) {
                try{
                    ...
                    Picasso.with(DetailReservationScreen.this).load(task.getResult().toString()).into(imgView);
        ...
        if(oferta.getComentario().equalsIgnoreCase("")) {
            if(Calendar.getInstance().getTime().after(oferta.getDia_fin())) {
                nueva_reseña.setVisibility(View.VISIBLE);
            }
        }
                ...
                if(ofertaFire.getId_residencia().equalsIgnoreCase(oferta.getId_residencia())) {
                    if (ofertaFire.getComentario() != null) {
                        if (!ofertaFire.getComentario().equalsIgnoreCase("")) {
                            list.add(ofertaFire);
                            adapter = new ComentarioAdapter(DetailReservationScreen.this, list);
    ...
    public void gestionarEventos(){
        nueva_reseña.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(DetailReservationScreen.this, AddCommentScreen.class);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == 1) {
            if(resultCode == Activity.RESULT_OK){
                Toast.makeText(this, getText(R.string.comentario_añadido), Toast.LENGTH_LONG).show();
            }
        }
    }
